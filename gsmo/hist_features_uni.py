...
def get_embeddings_uni2(img):
    ...
    return embedding.cpu().numpy().squeeze()  # [1536]

def extract_patch_embeddings(img, patch_size, stride):
    """预计算整个图像的滑动窗口 patch embedding"""
    h, w, _ = img.shape
    embeddings = {}
    ...
    for y in tqdm(range(0, h - patch_size + 1, stride), desc=f"Extracting {patch_size}x{patch_size} patches"):
        for x in range(0, w - patch_size + 1, stride):
            ...
            embeddings[(x, y)] = get_embeddings_uni2(patch)
...
def extract_embeddings(img, locs, rad, pixel_size_raw, pixel_size=0.5, pretrained=True,device='cuda'):
    """提取所有测序点的 embedding"""
    ...
    # 预计算 256x256 和 4096x4096 的 patch embedding
    embeddings_256 = extract_patch_embeddings(img, patch_size=256, stride=128)
    embeddings_4096 = extract_patch_embeddings(img, patch_size=4096, stride=2048)

    embeddings = np.zeros((locs.shape[0], 4608), dtype=np.float32)
    ...
    for i in tqdm(range(locs.shape[0]), desc="Extracting final embeddings"):
        ...
        emb_mid = find_closest_patch(embeddings_256, center_x, center_y, patch_size=256, stride=128)
